...
def save_state(state):
    # Keep a bounded history so the file stays small.
...
def fetch_entries():
    response = requests.get(
        RSS_URL,
        timeout=30,
        headers={"User-Agent": "main-tauber-news-monitor/1.0"},
    )
...
def translate_title(title):
    try:
        return GoogleTranslator(source="de", target="zh-CN").translate(title)
    except Exception as exc:
        print(f"Translation failed, falling back to German only: {exc}")
        return "（中文翻译暂时失败）"
...
def main():
    ...
    # RSS feeds are normally newest-first. Sorting by date makes multi-item
    ...
    if test_mode:
        # Send exactly one latest RSS item, regardless of publication date
        # or whether it was sent before.
    ...
    for entry in candidates:
        ...
        try:
            ...
            sent.add(key)
            # Persist after each successful push so a later failure cannot
            # cause already-sent items to be duplicated on the next run.
            save_state(state)
        except Exception as exc:
